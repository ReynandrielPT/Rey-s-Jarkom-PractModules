...
- Flag
  ...
  `JARKOM25{N1c3_0ne_b4nggg_ZCLSHSCDMOyuMM13yvxflsvghxttcntrbaqjkc3r4t0ps40854084276714868254_712130a81e29ba65be59609f6b248233}`
  <br>
  <img width="1451" height="536" alt="image" src="https://github.com/user-attachments/assets/b1d412bb-b391-4e75-9234-9147b0267aba" />
...
- Filter expression
  ...
  `cp.flags.ack == 1 && tcp.len == 0 `

- Explanation

  `Okeh keywordnya tcp murni dan ack, untuk mencari file tcp murni artinya kita perlu filter tcp tanpa data sedangkan ack, kita perlu filter file yang ada ack nya, dan karena diminta tcp DAN ack, artinya kita pakai operator &&, namun jumlah yang didapat yaitu 3210 adalah salah, karena kalau kita lihat lagi, ternyata ada 1 file tcp yang unseen segment, jadi total ada 3210 - 1 = 3209`

- Output result

  <img width="2794" height="105" alt="image" src="https://github.com/user-attachments/assets/e9beb683-6a71-43bb-9f76-79b54b558cb9" />
  <br>
  <img width="638" height="89" alt="image" src="https://github.com/user-attachments/assets/c1820094-1fea-4d0a-91d8-112e0927e4ed" />
...
- Explanation
  ...
  `Pada deskripsi, diminta untuk hanya mmencari paket hanya ack, bisa dilakukan dengan filter  tcp.flags == 0x10. 0x10 merupakan angka 16 dalam hex yaitu identity dari ack. Namun sama seperti sebelumnya ada 2 file yang unseen segment jadi 3174 - 2 = 3172`
- Output result

  <img width="638" height="89" alt="image" src="https://github.com/user-attachments/assets/0b6dff43-8cc8-474d-90fb-85c91172a7a9" />
  <br>
  <img width="2864" height="233" alt="image" src="https://github.com/user-attachments/assets/531aa17c-b018-4392-a847-4f1b8d1c5c9f" />
  <br>
  <img width="2864" height="233" alt="image" src="https://github.com/user-attachments/assets/071e14f6-b699-45da-8b0c-c54a22aa2000" />
...
- Explanation
  ...
  `Untuk soal ini, cukup melakukan negasi dari soal sebelumnya, namun hasil yang muncul adalah 48. pada saat ini saya mencoba bruto force dengan angka terdekat dan didapat jawabannya adalah 49`

- Output result

  <img width="559" height="53" alt="image" src="https://github.com/user-attachments/assets/15265c5e-d625-43fa-8771-fdb120c44f39" />
...
- Flag
  ...
  `JARKOM25{W0w_Y0uU_h4V33e_d0n3_444_90od_j0bB_MH69Zg0dl1k30x19fewcu9vvkqhfcohyvn_1a46620f20c1cb56281472b0b87361e0}` <br>
- <img width="1457" height="614" alt="image" src="https://github.com/user-attachments/assets/759ef5d0-5ef2-4704-b6ba-afaab46a1ab9" />
...
- Explanation
  ...
  `Pada soal kali ini, saya melakukan filter untuk telnet, dan melakukan follow -> stream. Terlihat sebuah page, pada page tersebut pada pojok kiri bawah, bisa dilakukan penyaringan, terlihat penyaringan pada port 54184, menampilkan bagian dimana halaman pembuka ketika client masuk via telnet, dan akhirnya itulah jawabannya`

- Output result

  <img width="1896" height="1547" alt="image" src="https://github.com/user-attachments/assets/ea503d19-28b1-4641-8607-bd4eeffafcf6" />
  <br>
...
- Explanation
  ...
  `Cara yang kurang lebih sama, seperti pertanyaan sebelumnya, filter telnet -> follow -> stream, pada page yaitu pada port 54184, terlihat page tersebut merupakan semua output yang diberikan dari server, itu berarti jawaban untuk soal ini, yaitu bytes yang ada di pojok kiri bawah, di samping port `

- Output result

  <img width="827" height="89" alt="image" src="https://github.com/user-attachments/assets/f218f9d2-a29b-4517-a676-154166eaa9d4" />
...
- Explanation
  ...
  `Di Page yang sama, yaitu output dari server, terlihat server sempat output username dari client, setelah client mengisi usernamenya yaitu jovyan`

- Output result

  `<img width="311" height="62" alt="image" src="https://github.com/user-attachments/assets/0b2577cc-f8e4-411f-9bfe-56a2008b5858" />
...
- Explanation
  ...
  `Untuk soal kali ini, pada page output server, tidak terlihat ada password, jadi kita coba ganti filternya dan menggabungkan output dari client dan server secara bersamaan dan jawaban di dapat`

- Output result

  <img width="612" height="154" alt="image" src="https://github.com/user-attachments/assets/23068145-2390-4f29-b185-d6a7aaff8e72" />
...
- Flag
  ...
  `JARKOM25{G04t__a4n4liz333er_0BDCT04BP5NUZ3YISP6Vfr0gpzu3ccnh1ukrllsr3ys0970628779_ce85330586edb9dcdf428a5bc2ed82b0}`
  <img width="1462" height="966" alt="image" src="https://github.com/user-attachments/assets/8cdb84a0-2248-47c3-9d3a-ff04d7ebcee4" />
...
- Explanation
  ...
  `Saya melakukan follow -> stream -> scroll, dan menemukan satu halaman yang menarik, yaitu halaman ketika client berhasil akses server. Saya mencoba filter bagian yang output dari server, dan terlihat bahwa command pertama client adalah echo`

- Output result

  <img width="1887" height="1420" alt="image" src="https://github.com/user-attachments/assets/28fc5b4b-038c-4e26-8244-4fb0cfaffd25" /> <br>
  <img width="1820" height="191" alt="image" src="https://github.com/user-attachments/assets/b1bdb23a-734c-4165-ba62-902a64830988" />
...
- Explanation
  ...
  `Menggunakan metode yang sama seperti soal sebelumnya, terlihat setelah echo, client melakukan cat pada file test.txt`

- Output result

  `<img width="347" height="110" alt="image" src="https://github.com/user-attachments/assets/a407bef2-da5f-4246-be35-4851420dcc76" />
...
- Explanation
  ...
  `masih sama dengan metode sebelumnya, setelah melakukan cat, terlihat client melakukan nano/pengeditan file pada file test.txt, dan kalimat tersebut adalah Jarkom gampang, tapi karena soal meminta kata pertama maka jawabannya adalah Jarkom`

- Output result

  <img width="1839" height="201" alt="image" src="https://github.com/user-attachments/assets/78267633-3ccc-4b80-9a87-9bab81bc7dcf" />
...
- Flag
  ...
  `JARKOM25{n4il0ng_m1lk_dr4g000n_FXXMJ9YTPI7F8JHMDUGWJVULCV38IRcr0cjazo2r51zaspfv9445uzb434_7d1cb2254f56e2488efa92d8ba9a6d39}`
  <br>
  <img width="1500" height="532" alt="image" src="https://github.com/user-attachments/assets/98091c08-0a9c-4860-8bd8-977e73b1c8ca" />
...
- Explanation
  ...
  `masukan filter http, dan terlihat jawabannya yaitu 298`

- Output result

  <img width="624" height="129" alt="image" src="https://github.com/user-attachments/assets/649ad34c-81cd-4444-ad61-3825c21dd449" />
...
- Explanation
  ...
  `Cukup masukan filter, dan hasilnya terlihat`

- Output result

  <img width="2871" height="1563" alt="image" src="https://github.com/user-attachments/assets/12e1aa29-cbc4-4f41-bef6-19056cf3a426" />
...
- Explanation
  ...
  `Pada soal kali ini kita diminta paket yang berhasil, jadi pertama kita masukan filter http, tapi jika kita scroll ada 2 paket http yang tidak berhasil jadi total 298 - 2 = 296`

- Output result

  <img width="2871" height="1563" alt="image" src="https://github.com/user-attachments/assets/fa551014-0ac6-41c0-ac83-fae083e97707" />
...
- Explanation
  ...
  `Karena diminta IP dari client, kita lakukan filter http.responce untuk melihat responce dan request dari client, kita lihat IP source karena itu adalah IP asal (IP yang di request dari client)`

- Output result

  <img width="339" height="208" alt="image" src="https://github.com/user-attachments/assets/2421af19-0665-43be-a34b-c39d57ea333a" />
...
- Explanation
  ...
  `Untuk soal kali ini karena ini gambar yang di request dari client, seharusnya akan ada GET {gambar}, sehingga saya follow -> stream -> scrolling, hingga menemukan request GET yang "menarik"`

- Output result

  <img width="1007" height="571" alt="image" src="https://github.com/user-attachments/assets/5c1bebba-e3a1-4db0-af0c-ac9d0869f6a4" />
...
- Explanation
  ...
  `Untuk mencari ip addres dari FTP, kita filter, dengan menggunakan perintah "ftp", disini ada responce dan request, karena yang diminta IP addres dari server, jadi saya lihat IP source dari Responce, karena Responce pesan yang dikirim dari server -> client`
